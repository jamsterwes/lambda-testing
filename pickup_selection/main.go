...
// ByTime implements sort.Interface for []RouteSummary based on
// the Time field.

func RankPickupPoints(inboundSummaries []RouteSummary, outboundSummaries []RouteSummary, maxPoints int) []RouteSummary {
	// sort.Sort(ByTime(outboundSummaries))
	return outboundSummaries
	//return []RouteSummary{}
}
...
func HandleRequest(ctx context.Context, event *PickupSelectionRequest) (*PickupSelectionResponse, error) {
	...
	// RankedOutboundSummaries := RankPickupPoints(inboundSummaries, outboundSummaries, 10)
	sort.Slice(outboundSummaries, func(i, j int) bool {
		return outboundSummaries[i].Time < outboundSummaries[j].Time
	})
